# End-to-end GliderADCP.jl workflow, run symmetrically over any sea064 mission.
#
# Native .ad2cp binary (with a binary-vs-MIDAS-netCDF parity check where an export
# exists) + multi-route SeaExplorer loading (glider-computer files and the
# GLIMPSE-server export merged and deduplicated) → sound-speed correction → QC →
# declination (IGRF) → ENU pings → shear-bias calibration → DAC + screened bottom
# track → inverse, shear & vertical solutions → diagnostics → gridded sections
# (netCDF/CSV) → figures. Every mission is treated identically; latitude and the
# stream counter come from the registry / navigation, nothing is hard-coded per
# mission in the body.
#
# Run (CairoMakie is found in the user's @ocean environment via the stacked load
# path; the package itself stays plot-free):
#
#   JULIA_LOAD_PATH="@:@ocean:@stdlib" julia +1.13 --project=. examples/currents.jl
#   JULIA_LOAD_PATH="@:@ocean:@stdlib" julia +1.13 --project=. examples/currents.jl m38
...
# invert one time window (half-yo) independently — for the dive-vs-climb check
...
function process(m)
    ...
    @info "1/7 load AD2CP (native binary) + navigation (delayed + GLIMPSE, deduplicated)"
    adcp = read_ad2cp(joinpath(m.dir, m.binary))
    if m.netcdf !== nothing                              # reader-parity check where a MIDAS export exists
        nc = load_ad2cp(joinpath(m.dir, m.netcdf))
        dv = length(nc) == length(adcp) ?
             maximum(abs, filter(!isnan, vec(nc.vel .- adcp.vel))) : NaN
        @info "    binary vs MIDAS netCDF: $(length(nc)) ens, max |Δvel| = $dv"
    end
    @info "    $adcp"
    cov = coverage(adcp)
    ...
    @info "    $(length(nav)) nav rows, mean latitude $(lat)°N"

    @info "2/7 sound-speed correction from payload CTD (pld1.sub, delayed + GLIMPSE)"
    pld = load_seaexplorer_pld([joinpath(m.dir, "delayed/pld1/logs"), joinpath(m.dir, "glimpse")];
    ...
    winv = solve_w(pings, dac; method=:inverse)      # w, pressure-anchored inverse
    @info "    inverse: $(length(unique(inv.yo))) yos, $(nrow(inv)) bins; " *
          "shear: $(length(unique(shr.yo))) yos; w: $(length(unique(wdir.yo))) yos"

    @info "6/7 diagnostics (the workflow health checks)"
    ...
    for row in eachrow(dac[1:2:end, :])          # every other yo
        ...
        for (k, z) in enumerate(sd.z)
            ...
            push!(dc_u, (sd.u[k], sc.u[kk])); push!(dc_v, (sd.v[k], sc.v[kk]))
        end
    end
    ...
    sec = grid_profiles(inv); sec_s = grid_profiles(shr)
    sec_wd = grid_profiles(wdir; fields=(:w, :w)); sec_wi = grid_profiles(winv; fields=(:w, :w))
    attrs = Dict{String,Any}("mission" => m.label,
        "instrument" => "Nortek Glider AD2CP 1 MHz SN102381",
        "declination" => "IGRF per ping",
        "qc_rejected_fraction" => round(qstats.total, digits=4))
    export_sections(joinpath(OUT, "$(m.label)_sections_inverse.nc"), sec;
        attrs=merge(attrs, Dict{String,Any}("method" => "Visbeck inverse, DAC + bottom track")))
    export_sections(joinpath(OUT, "$(m.label)_sections_shear.nc"), sec_s;
        attrs=merge(attrs, Dict{String,Any}("method" => "shear + DAC referencing")))
    CSV.write(joinpath(OUT, "$(m.label)_profiles_inverse.csv"), inv)
    CSV.write(joinpath(OUT, "$(m.label)_profiles_shear.csv"), shr)
    CSV.write(joinpath(OUT, "$(m.label)_profiles_w_direct.csv"), wdir)
    CSV.write(joinpath(OUT, "$(m.label)_profiles_w_inverse.csv"), winv)
    ...
    save(joinpath(OUT, "$(m.label)_UV_sections.png"), fig)
    figw = plot_sections([(sec_wd, :U, "w — direct (U_rel + dP/dt, binned)"),
                          (sec_wi, :U, "w — inverse (pressure-anchored)")];
        colorrange=(-crW, crW))
    save(joinpath(OUT, "$(m.label)_w_sections.png"), figw)

    fig2 = Figure(size=(1500, 460))
    ax1 = Axis(fig2[1, 1]; title=@sprintf("shear vs inverse (u): r=%.2f, rms=%.3f", r_u, rms_u),
        xlabel="inverse u (m/s)", ylabel="shear u (m/s)", aspect=1)
    scatter!(ax1, j.u[gm], j.u_1[gm]; markersize=2, color=(:steelblue, 0.25)); ablines!(ax1, 0, 1; color=:black)
    ax2 = Axis(fig2[1, 2]; title=@sprintf("dive vs climb (u): r=%.2f, med|Δ|=%.3f", rdc_u, mad_u),
        xlabel="dive u (m/s)", ylabel="climb u (m/s)", aspect=1)
    scatter!(ax2, first.(dc_u), last.(dc_u); markersize=3, color=(:darkorange, 0.35)); ablines!(ax2, 0, 1; color=:black)
    ax3 = Axis(fig2[1, 3]; title="per-yo DAC closure", xlabel="yo", ylabel="|Δ| (m/s)")
    scatter!(ax3, eachindex(clos), clos; markersize=4, color=:seagreen)
    save(joinpath(OUT, "$(m.label)_diagnostics.png"), fig2)
    @info "    wrote $(m.label)_{UV,w}_sections.png, $(m.label)_diagnostics.png, sections + profiles"
end

using CairoMakie
for key in selected_missions()
    process(MISSIONS[key])
end
println("\nOutputs in $(OUT):")
